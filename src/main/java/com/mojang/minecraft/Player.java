package com.mojang.minecraft;

import com.mojang.minecraft.level.Level;
import org.lwjgl.input.Keyboard;

public class Player extends Entity {
    public static final int KEY_UP = 0;
    public static final int KEY_DOWN = 1;
    public static final int KEY_LEFT = 2;
    public static final int KEY_RIGHT = 3;
    public static final int KEY_JUMP = 4;
    private boolean[] keys = new boolean[10];
    public static float speed = 0.02f;
    public static float speedAir = 0.1F;
    public static float fallSpeed = 0.08f;

    public Player(Level level) {
        super(level);
        this.heightOffset = 1.62F;
    }

    public void setKey(int key, boolean state) {
        int id = -1;

        if (key == Keyboard.KEY_UP || key == Keyboard.KEY_W)
            id = KEY_UP;
        if (key == Keyboard.KEY_DOWN || key == Keyboard.KEY_S)
            id = KEY_DOWN;
        if (key == Keyboard.KEY_LEFT || key == Keyboard.KEY_A)
            id = KEY_LEFT;
        if (key == Keyboard.KEY_RIGHT || key == Keyboard.KEY_D)
            id = KEY_RIGHT;
        if (key == Keyboard.KEY_SPACE)
            id = KEY_JUMP;

        if (0 <= id) this.keys[id] = state;
    }

    public void releaseAllKeys() {
        for (int i = 0; i < 10; i++) {
            this.keys[i] = false;
        }
    }

    @Override
    public void tick() {
        this.xo = this.x;
        this.yo = this.y;
        this.zo = this.z;
        float xa = 0.0F;
        float ya = 0.0F;
        boolean inWater = this.isInWater();
        boolean inLava = this.isInLava();
        if (this.keys[KEY_UP]) ya--;
        if (this.keys[KEY_DOWN]) ya++;
        if (this.keys[KEY_LEFT]) xa--;
        if (this.keys[KEY_RIGHT]) xa++;

        if (this.keys[KEY_JUMP]) {
            if (inWater) {
                this.yd += 0.04F;
            } else if (inLava) {
                this.yd += 0.04F;
            } else if (this.onGround) {
                this.yd = 0.42F;
                this.keys[4] = false;
            }
        }

        if (inWater) {
            float yo = this.y;
            this.moveRelative(xa, ya, speed);
            this.move(this.xd, this.yd, this.zd);
            this.xd *= 0.8F;
            this.yd *= 0.8F;
            this.zd *= 0.8F;
            this.yd = (float)(this.yd - speed);
            if (this.horizontalCollision && this.isFree(this.xd, this.yd + 0.6F - this.y + yo, this.zd)) {
                this.yd = 0.3F;
            }
        } else if (inLava) {
            float yo = this.y;
            this.moveRelative(xa, ya, speed);
            this.move(this.xd, this.yd, this.zd);
            this.xd *= 0.5F;
            this.yd *= 0.5F;
            this.zd *= 0.5F;
            this.yd = (float)(this.yd - speed);
            if (this.horizontalCollision && this.isFree(this.xd, this.yd + 0.6F - this.y + yo, this.zd)) {
                this.yd = 0.3F;
            }
        } else {
            this.moveRelative(xa, ya, this.onGround ? speedAir : speed);
            this.move(this.xd, this.yd, this.zd);
            this.xd *= 0.91F;
            this.yd *= 0.98F;
            this.zd *= 0.91F;
            this.yd = (float)(this.yd - fallSpeed);
            if (this.onGround) {
                this.xd *= 0.6F;
                this.zd *= 0.6F;
            }
        }
    }
}
